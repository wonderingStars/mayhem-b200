...
//
// sdrlink is meant to run on an isolated machine with no network access for
// `go get`, and the whole point of the product is a single self-contained
// binary — so the WebSocket endpoint that streams spectrum frames is built
// on net/http's Hijacker rather than pulling in a third-party library. It
// implements exactly what this server needs: unmasked server->client frames,
// masked client->server frame reading, ping/pong/close. It does not attempt
// to be a general-purpose WebSocket client library (no fragmentation
// reassembly of client frames, no permessage-deflate) because the only
// client this endpoint ever talks to is a browser <canvas> that receives
// spectrum frames and occasionally acks a ping.
...
var (
	errNotUpgrade  = errors.New("web: not a websocket upgrade request")
	errNoHijack    = errors.New("web: response writer does not support hijacking")
	errFrameTooBig = errors.New("web: websocket frame exceeds maximum size")
)

// maxClientFrame bounds how much a client frame's declared length we will
// allocate for, so a malicious/buggy client can't make us allocate
// arbitrary amounts of memory from a forged length field.
const maxClientFrame = 1 << 20 // 1 MiB, generous for a ping/pong/close peer
...
// it. This server does not need to accept data frames from the browser, so
// fragmented (FIN=0) frames are treated as a protocol error rather than
// reassembled.
func (c *wsConn) ReadMessage() (opcode byte, payload []byte, err error) {
	...
	if !fin {
		return 0, nil, fmt.Errorf("web: fragmented client frames are not supported")
	}
