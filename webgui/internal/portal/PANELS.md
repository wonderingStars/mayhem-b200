...
publish `geotable`, which mounts this renderer as its upper half (radiosonde and
EPIRB are queued behind the same kind). The basemap reaches all of them through
that one renderer: `geotable.js` composes `map.js` and needed no change to gain it.
